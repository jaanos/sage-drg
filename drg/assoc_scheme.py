...
class ASParameters:
    ...
    def _subs(self, exp, p):
        ...
        """
        if "p" in self.__dict__ and "p" not in p.__dict__:
            p.p = self.p.subs(*exp)
        if "q" in self.__dict__ and "q" not in p.__dict__:
            p.q = self.q.subs(*exp)
        if "P" in self.__dict__ and "P" not in p.__dict__:
            p.P = self.P.subs(*exp)
        if "Q" in self.__dict__ and "Q" not in p.__dict__:
        ...
        while len(check) > 0:
            for t in list(check):
                ...
                for d in list(zero[t]):
                    ...
                    try:
                        sol = sort_solution(
                            next(g[t].send((True, self.triple[t][d] >= 1))))
                        if sol not in r[t]:
...
class PolyASParameters(ASParameters):
    ...
    def __init__(self, b, c=None, order=None):
        ...
        self.hash_parameters = self.parameterArray(factor=True, simplify=2)
        self._init_prefix()
    ...
    def _compute_parameters(self, p, k):
        ...
        for i in range(2, self.d + 1):
            for j in range(1, self.d + 1):
                ...
                p[self.d, i, j] = self._check_parameter(
                    h, i, j,
                    _simplify(_expand((
